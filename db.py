...
def test_middleman_storage_example():
    """
    Example function showing how middleman LLM invocations will be stored.
    This is for documentation/testing purposes only.
    """
    logger.info("=== MIDDLEMAN STORAGE EXAMPLE ===")
    
    # Example of how a two-step workflow would store invocations:
    example_account = "test-account-123"
    example_conversation = "conv-456"
    example_invocation = "inv-789"  # Same for both steps
    
    # Step 1: Middleman LLM call would store:
    middleman_example = {
        'associated_account': example_account,
        'input_tokens': 150,
        'output_tokens': 80,
        'llm_email_type': 'intro_email_middleman',
        'model_name': 'meta-llama/Llama-3.3-70B-Instruct-Turbo-Free',
        'conversation_id': example_conversation,
        'invocation_id': example_invocation
    }
    
    # Step 2: Output LLM call would store:
    output_example = {
        'associated_account': example_account,
        'input_tokens': 200,  # Larger because it includes middleman instructions
        'output_tokens': 60,
        'llm_email_type': 'intro_email',
        'model_name': 'meta-llama/Llama-3.3-70B-Instruct-Turbo-Free',
        'conversation_id': example_conversation,
        'invocation_id': example_invocation  # Same invocation_id groups them together
    }
    
    logger.info("Example middleman invocation would be stored as:")
    logger.info(f"  - Type: {middleman_example['llm_email_type']}")
    logger.info(f"  - Tokens: {middleman_example['input_tokens']} + {middleman_example['output_tokens']} = {middleman_example['input_tokens'] + middleman_example['output_tokens']}")
    logger.info(f"  - Is Middleman: True")
    logger.info(f"  - Base Scenario: intro_email")
    logger.info(f"  - Invocation ID: {middleman_example['invocation_id']}")
    
    logger.info("Example output invocation would be stored as:")
    logger.info(f"  - Type: {output_example['llm_email_type']}")
    logger.info(f"  - Tokens: {output_example['input_tokens']} + {output_example['output_tokens']} = {output_example['input_tokens'] + output_example['output_tokens']}")
    logger.info(f"  - Is Middleman: False")
    logger.info(f"  - Base Scenario: intro_email")
    logger.info(f"  - Invocation ID: {output_example['invocation_id']} (SAME - groups them together)")
    
    total_tokens = (middleman_example['input_tokens'] + middleman_example['output_tokens'] + 
                   output_example['input_tokens'] + output_example['output_tokens'])
    
    logger.info(f"Total tokens for complete two-step workflow: {total_tokens}")
    logger.info(f"Both records share invocation_id '{example_invocation}' for grouping")
    logger.info("=== END MIDDLEMAN STORAGE EXAMPLE ===")
